...
class DecoderRequest(BaseModel):
    ...
    config: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Decoding configuration"
    )
...
class DecoderResponse(BaseModel):
    ...
    confidence: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Overall decoding confidence"
    )
...
class EmotionalState(BaseModel):
    ...
    emotion_scores: Dict[str, float] = Field(
        default_factory=dict, description="Scores for all emotion categories"
    )
...
class WorldState(BaseModel):
    ...
    object_density: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Object placement density"
    )
    structure_level: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Level of structural organization"
    )
    ...
    change_rate: float = Field(
        default=0.1, ge=0.0, le=1.0, description="Rate of environmental changes"
    )
    morph_speed: float = Field(
        default=1.0, ge=0.1, le=5.0, description="World morphing speed multiplier"
    )
...
class DecoderConfig(BaseModel):
    ...
    model_path: str = Field(
        default="/app/models/checkpoints", description="Path to model checkpoints"
    )
    ...
    ensemble_weights: List[float] = Field(
        default_factory=list, description="Weights for ensemble models"
    )
...
class TrainingData(BaseModel):
    ...
    targets: List[List[float]] = Field(
        ..., description="Target CLIP-style embeddings for EEG-to-CLIP training"
    )
...
class InferenceRequest(BaseModel):
    ...
    model_type: str = Field(
        ..., description="Type of model (eeg_to_clip, emotion_classifier, motif_detector)"
    )
