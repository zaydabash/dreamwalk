...
from models.decoder_models import DecoderConfig, EmotionalState, TrainingData
...
class EmotionClassifierNet(nn.Module):
    ...
    def __init__(self, input_dim: int, num_classes: int, hidden_dims: List[int] = None):
        ...
        self.emotion_classifier = nn.Sequential(
            nn.Linear(prev_dim, 64), nn.ReLU(), nn.Dropout(0.2), nn.Linear(64, num_classes)
        )
...
class EmotionClassifier:
    ...
    async def train_synthetic(self, training_data: TrainingData):
        ...
        try:
            self.logger.info(
                "Starting emotion classifier synthetic training",
            ...
            val_dataset = EmotionDataset(
                X_val, y_val, scaler=train_dataset.scaler, label_encoder=train_dataset.label_encoder
            )
            ...
            train_dataset = EmotionDataset(
                X_train, y_train, scaler=self.scaler, label_encoder=self.label_encoder
            )
            val_dataset = EmotionDataset(
                X_val, y_val, scaler=self.scaler, label_encoder=self.label_encoder
            )
