    ...
    def validate_texture_types(cls, v):
        ...
        for texture_type in v:
            if texture_type not in valid_types:
                raise ValueError(f'Invalid texture type: {texture_type}. Must be one of {valid_types}')
        return v
...
class TextureGenerationConfig(BaseModel):
    ...
    model_name: str = Field(default="runwayml/stable-diffusion-v1-5", description="Diffusion model name")
...
class StylePreset(BaseModel):
    ...
    negative_prompt: Optional[str] = Field(
        default=None,
        description="Override negative prompt for this preset"
    )
