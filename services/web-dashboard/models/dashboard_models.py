...
class ServiceStatus(BaseModel):
    ...
    last_check: datetime = Field(
        default_factory=datetime.utcnow, description="Timestamp of last health check"
    )
...
class DashboardData(BaseModel):
    ...
    recent_world_states: List[Dict[str, Any]] = Field(
        default_factory=list, description="Most recent world states"
    )
    services_status: Dict[str, ServiceStatus] = Field(
        default_factory=dict, description="Status of backend services"
    )
    system_metrics: Dict[str, Any] = Field(
        default_factory=dict, description="Redis/system level metrics"
    )
...
class SessionMetrics(BaseModel):
    ...
    active_motifs: List[str] = Field(
        default_factory=list, description="Distinct active neural motifs"
    )
...
class WorldStateHistory(BaseModel):
    ...
    session_id: str = Field(..., description="Session identifier")
    world_states: List[Dict[str, Any]] = Field(
        default_factory=list, description="World state snapshots"
    )
    timestamps: List[datetime] = Field(
        default_factory=list, description="Timestamps for each snapshot"
    )
...
class EEGSignalData(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="Sample window timestamp"
    )
