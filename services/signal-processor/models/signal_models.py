...
class EEGConfig(BaseModel):
    ...
    channels: List[str] = Field(
        default_factory=lambda: ["Fp1", "Fp2", "F3", "F4", "C3", "C4", "P3", "P4"],
...
class ProcessedFeatures(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="Feature extraction timestamp"
    )
...
class NeuralMotif(BaseModel):
    ...
    motif_type: str = Field(
        ..., description="Type of motif (e.g., 'meditation', 'stress', 'focus')"
    )
...
class ProcessingConfig(BaseModel):
    ...
    min_quality_threshold: float = Field(
        default=0.3, description="Minimum signal quality threshold"
    )
    ...
    connectivity_freq_bands: List[List[float]] = Field(
        default=[[8, 13], [13, 30]], description="Frequency bands for connectivity analysis"
    )
