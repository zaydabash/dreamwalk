...
class EEGProcessor:
    ...
    async def process_batch(self, raw_data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            processed_df = pd.DataFrame(
                processed.data.T, columns=processed.channels, index=raw_data.index
            )
    ...
    async def _fit_ica(self, data: np.ndarray, sampling_rate: float):
        ...
        try:
            ...
            info = mne.create_info(
                ch_names=self.config.channels[: data.shape[0]], sfreq=sampling_rate, ch_types="eeg"
            )
            ...
            self.logger.info(
                f"ICA fitted with {n_components} components, "
    ...
    def detect_artifacts(self, data: np.ndarray, sampling_rate: float) -> Dict[str, Any]:
        ...
        artifacts = {"eye_blinks": 0, "muscle_artifacts": 0, "flat_channels": 0, "bad_channels": []}
        ...
        try:
            ...
            high_freq = filter_data(data, sfreq=sampling_rate, l_freq=20, h_freq=45, method="fir")
