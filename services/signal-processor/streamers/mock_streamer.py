...
class MockStreamer:
    ...
    async def _generate_channel_signal(
        self, n_samples: int, channel: str, state_params: Dict[str, float], ch_idx: int
    ) -> np.ndarray:
        ...
        try:
            ...
            for band, (low_freq, high_freq) in self.frequencies.items():
                ...
                for _ in range(n_components):
                    ...
                    amplitude = (
                        ch_params["base_amplitude"] * power_multiplier * random.uniform(0.1, 0.5)
                    )
    ...
    async def _add_artifacts(self, data: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            if random.random() < 0.3:  # 30% chance of eye blink
                ...
                frontal_channels = [
                    i for i, ch in enumerate(self.eeg_config.channels) if ch.startswith("F")
                ]
    ...
    async def _update_emotional_state(self):
        ...
        try:
            ...
            if random.random() < self.state_transition_prob:
                available_states = [
                    s for s in self.emotional_states.keys() if s != self.current_state
                ]
                if available_states:
                    new_state = random.choice(available_states)
                    self.logger.info(
                        f"Emotional state transition: {self.current_state} -> {new_state}"
                    )
